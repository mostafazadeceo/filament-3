...
class MailtrapSandboxSendClient
{
    ...
    public function sendEmail(int $inboxId, array $payload): array
    {
        $response = $this->request('POST', "api/send/{$inboxId}", [], $payload, 'sendSandboxEmail');
    ...
    protected function request(string $method, string $resource, array $query = [], ?array $payload = null, ?string $endpointName = null): Response
    {
        ...
        try {
            ...
            throw MailtrapApiException::fromResponse(null, null, 'ارسال ایمیل به سندباکس Mailtrap ناموفق بود.');
        }
    }
    ...
    {
        $base = (string) config('mailtrap-core.sandbox_send_base_url', 'https://sandbox.api.mailtrap.io');
    ...
    protected function logRequest(
        ...
        logger()->info('mailtrap.sandbox.send', [
