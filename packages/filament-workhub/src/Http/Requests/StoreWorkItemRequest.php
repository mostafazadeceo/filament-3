...
class StoreWorkItemRequest extends FormRequest
{
    ...
    {
        return [
            'project_id' => ['required', 'exists:workhub_projects,id'],
            'work_type_id' => ['nullable', 'exists:workhub_work_types,id'],
            'status_id' => ['nullable', 'exists:workhub_statuses,id'],
            'title' => ['required', 'string', 'max:255'],
            'description' => ['nullable', 'string'],
            'priority' => ['required', Rule::in(array_keys(config('filament-workhub.work_item.priorities', [])))],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($values !== []) {
                [, $errors] = app(CustomFieldManager::class)->validateValues('work_item', $values, (int) $tenantId, true);
            ...
            if ($project && $this->filled('status_id')) {
