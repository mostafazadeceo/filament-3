...
class UpdatePurchaseOrder extends BaseRestaurantOpsRequest
{
    public function authorize(): bool
    {
        ...
        return $order
            ? auth()->user()?->can('update', $order) ?? false
            : auth()->user()?->can('update', RestaurantPurchaseOrder::class) ?? false;
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'purchase_request_id' => [
                'nullable',
                'integer',
                Rule::exists('restaurant_purchase_requests', 'id')
                    ->when($tenantId, fn ($rule) => $rule->where('tenant_id', $tenantId)),
            ],
