...
- اگر پکیج Plugin دارد، ثبت در پنل‌ها:
  - Admin: ثبت شده با CommerceCatalogPlugin::make() در AdminPanelProvider
  - Tenant: ثبت شده با CommerceCatalogPlugin::make() در TenantPanelProvider
...
- انتشار تنظیمات (در صورت نیاز):
  - `php artisan vendor:publish --tag=commerce-catalog-config`
## تست/اعتبارسنجی
